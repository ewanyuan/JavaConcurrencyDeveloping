...
public class MultiThreadNIOEchoServer {
     ...
     * queue and set the selector to thread.notify on OP_WRITE.
     */
    private void doRead(SelectionKey sk) throws IOException {
        SocketChannel channel = (SocketChannel) sk.channel();
        ByteBuffer buffer = ByteBuffer.allocate(8192);
        StringBuilder inputSp = new StringBuilder();
        ...
        //doWrite(sk);
        String input = inputSp.toString();
        double result = new Calculator().calculate(input);
        //write to channel
        ByteBuffer writeBuffer = Charset.forName("utf8").encode(input+"="+result);
        channel.write(writeBuffer);

        disconnect(sk);

        //tp.execute(new HandleMsg(sk, buffer));
    }
